...
c = gpd.read_file('data2/연도별.shp')
co = c.to_crs(4326)

df_table = pd.DataFrame({
    '서울시 용도별 건물 정보 데이터': ['국가공간정보포털 국가중점데이터'],
    '서울시 침수 흔적도': ['서울 열린데이터광장'],
    '하천/용도구역': ['국가공간정보포털 오픈마켓']
})

gdf_river = gpd.read_file('data2/LSMD_CONT_UJ201_11_202311.shp')

one = pd.read_csv("data2/서울172022-1.csv", encoding='euc-kr')
onee = pd.read_csv("data2/서울172022-2.csv", encoding='euc-kr')
oneee = pd.read_csv("data2/서울172022-3.csv", encoding='euc-kr')

two = pd.read_csv("data2/서울1819-1.csv", encoding='euc-kr')
twoo = pd.read_csv("data2/서울1819-2.csv", encoding='euc-kr')
twooo = pd.read_csv("data2/서울1819-3.csv", encoding='euc-kr')

three = pd.read_csv("data2/서울22-1.csv", encoding='euc-kr')
threee = pd.read_csv("data2/서울22-2.csv", encoding='euc-kr')
...
df_building = gpd.read_file('data2/최최종point.shp')
gdf_seoul_emd = gpd.read_file('data2/서울시경계.shp')
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        따라서 앞서 보았던 총 침수 주택 수 바 차트에서 보았던 것 처럼
                        
                        영등포구, 강동구, 강북구, 도봉구를 우선적으로 개선할 수 있도록 하는 방안을 마련해야 한다.
...
def update_statistics(selected_year):
    ...
    bar_chart = {
        ...
        'layout': {'title': '총 침수 주택 수'}
    }

    statistics_text = f"전체 데이터 기준 총 침수 주택 수:\n{statistics}"
...
def update_choropleth_map(selected_year):
    ...
    count1 = gpd.read_file('data2/영등포구없.shp')
    count2 = gpd.read_file('data2/최종카운트.shp')
